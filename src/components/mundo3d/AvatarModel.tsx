...
export interface PersonalizacionAvatar {
  ...
  // Estructura extendida parametrizada
  ropa?: {
...
// ---------------------------------------------------------------------
// 🎨 GENERADOR DINÁMICO DE TEXTURE ATLAS PARA ROSTROS (1 Draw Call / Shared Memory)
// ---------------------------------------------------------------------
...
function obtenerTextureAtlasRostro(): THREE.CanvasTexture {
  ...
  if (ctx) {
    ...
    dibujarCelda(0, 0, 'abierto', 'sonrisa');       // Alegre
    dibujarCelda(1, 0, 'guino', 'sonrisa');         // Guiño
    dibujarCelda(2, 0, 'abierto', 'seria');         // Serio
    dibujarCelda(3, 0, 'sorprendido', 'sorprendida'); // Sorprendido

    dibujarCelda(0, 1, 'cerrado', 'sonrisa');       // Pestañeo (Ojos cerrados)
    dibujarCelda(1, 1, 'cerrado', 'seria');         // Pestañeo Serio
    dibujarCelda(2, 1, 'cerrado', 'sorprendida');   // Pestañeo Sorprendido
    dibujarCelda(3, 1, 'abierto', 'sonrisa');       // Extra
  }
...
// ---------------------------------------------------------------------
// 👕 HOOK DE MATERIAL DE ROPA DUAL CON SHADER ULTRA-ROBUSTO
// ---------------------------------------------------------------------
...
export const AvatarModel: React.FC<AvatarModelProps> = ({
  ...
  // Animaciones procedurales: pestañeo aleatorio
  ...
  // Normalización de propiedades con retrocompatibilidad
  ...
  // Materiales de ropa parametrizados independientes (garantiza renderizado total)
  ...
  // Textura atlas clonada para control de UV offsets independiente por avatar
  ...
  // Actualizador de offset UV según expresión facial y pestañeo
  ...
  useFrame((state, delta) => {
    ...
    // 1. 👁️ Pestañeo Aleatorio Procedural
    ...
    if (isLocal) {
      if (!estaSentado) {
        ...
        if (piernaDerRef.current) piernaDerRef.current.rotation.x = THREE.MathUtils.lerp(piernaDerRef.current.rotation.x, -Math.PI / 2.2, suavizadoSentado);
        if (piernaIzqRef.current) piernaIzqRef.current.rotation.x = THREE.MathUtils.lerp(piernaIzqRef.current.rotation.x, -Math.PI / 2.2, suavizadoSentado);
      }
    ...
    // 2. 🫁 Idle Bobbing (Respiración Procedural) & 🚶 Toy Walk Waddle
    if (cuerpoRef.current) {
      if (estaCaminando) {
        ...
        cuerpoRef.current.position.y = bobY;
        ...
        cuerpoRef.current.position.y = 0;
  ...
  return (
    <group ref={grupoRef} position={position} rotation={rotation}>
      <group ref={cuerpoRef} scale={escala}>
        ...
        {/* Torso con Shader de Tinte Dual (Primario / Secundario) */}
        ...
        <group>
          ...
          {/* Plano de Rostro UV Atlas Expressivo */}
...
function esPosicionValida(pos: THREE.Vector3, isAula: boolean = false): boolean {
  ...
  if (isAula) {
    ...
    if (Math.abs(x) < 2.2 && z >= -14.8 && z <= -11.2) return false;
